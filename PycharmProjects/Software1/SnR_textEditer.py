...
main_application.title('SnR text Editor')
...
tool_bar.pack(side=tk.TOP,fill=tk.X)  #for horizontal X,verticle Y,both fill both
...
text_editer.configure(font=('Arial',16))
...
def changed(event=None):
    ...
    if text_editer.edit_modified():
        ...
        character=len(text_editer.get(1.0,'end-1c'))
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            mbox=messagebox.askyesnocancel('Warning','Do u want to save file ?')
            if mbox is True:
                if url:
                    content=text_editer.get(1.0,tk.END)
                    with open(url,'w',encoding='utf-8') as fw:
...
file.add_command(label='Exit',image=exit_icon,compound=tk.LEFT,accelerator='Alt+X',command=exit_func)
...
edit.add_command(label='Find',image=find_icon,compound=tk.LEFT,accelerator='Alt+F',command=find_func)
...
def change_theme():
    ...
    text_editer.config(background=bg_color,foreground=fg_color)
...
main_application.bind('<Alt-f>',find_func)
